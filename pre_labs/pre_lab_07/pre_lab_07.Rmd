...
```{r echo=FALSE}
census_api_key("6c6f00214ef0eb2cab5b4ff78cf0bd45c520fec3", install=TRUE)
```
...
### Task 3: Examining Census dreaata
...
It's Nevada with a percentage growth of 35.15%. 
```{r}
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Estimated median household income by county in Maryland.
...
#Calvert and Montgomery are fighting for second place because the margin of error overlaps.

Is the difference between Calvert and Montgomery significant? **Answer** No, the difference is not significant.
...
**Task** Run the following codeblock to join the data, rename the population estimates and then calculate the values we need. Notice how we select *after* we do the initial calculations so we only see the columns we're interested in (and drop the ones we don't want to see). Also note how the select function has the minus sign (-) in front of some columns? That means we don't want to see them in our result. Which jurisdictions saw the largest increases in median income? Did any counties see their median incomes fall from 2010-2021? **Answer** 
 The jurisdictions that saw the largest increases in median income is Baltimore City with 37.4% increase, Allegany County with 35.3% increase, Calvert County with 32.4%, Frederick County with 29.9% and Anne Arundel County with 29.5%. No, there aren't any counties that see their median incomes fall from 2010-2021.
```{r}
md10 |>
...
There are 50 high schools in zip codes where the average median household income is $111,448.30 that have 60% or more graduates immediately enrolling in college. There are 76 high schools in zip codes where the average median household income is $69,252.24 that have 40% and less graduates immediately enrolling in college.
```{r}
...
What is the name and county of the school at the top of the 50 high schools with he average median household income is $111,448.30 that have 60% or more graduates immediately enrolling in college and what is the name and county of the high school at the bottom of the 76 high schools in zip codes where the average median household income is $69,252.24 that have 40% and less graduates immediately enrolling in college. Then how many students graduated from each of those schools that year.