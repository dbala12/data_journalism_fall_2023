...
The datasets have 9 columns each and they seem to have the same structure. However, there are some differences in that the order of the columns for GRN, LIB, UNA and  OTH columns are positioned differently in each year. All the headers for the columns are standardized and capitalized, except for the total header column in the 2022 data set, which is Total insterad of TOTAL.
```{r}
...
It combines the dataframes and the rows.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** It seems that the total number of voters have increased from 2016 to 2022. The number of democratic voters have increased by almost 18,000 and number republican voters have risen by almost 4,000. The number of voters for other parties have also increased significantly by 1,000 from 2016 to 2022. This may show that the population in Frederick county is growing.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** The information in the demographic data includes a column with geocode information for each county, which can be useful in joining columns which have different names. There are columns that describe the demographics of the population per county too such number of people over 18 and population according to race.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer**The county with the highest 18+ Black population as a percentage at 60.9% is Prince George's County and this county also has the highest percentage of democrats too at 77.1%. This is followed by Baltimore City and then Charles county.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The county with the highest 18= Asian population as percentage at 19.4% is Howard county, with the next county being Montgomery. The county with the highest percentage of Democrats in the county is still Pronce George's. Compared to the previous task Howard and Montgomery are at the top 2 highest percentage of 18+ Asian population but overall in the last data set, was in the 8th and 9th highest percentage of 18+ Black population and 2nd and 3rd last highest percentage of Democrats. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** It's a numeric data type: dbl.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** It is now a character data type.
