...
**Task** Run the following code and describe the data frame it outputs. **Answer** The dataframe has 4 columns and 999 rows of information. The 4 columns are broken down to the date and title of the press releases followed by the URL for the press releases. The 4th column titled 'text' is basically the text written in the press releases.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The unique function lists down unique words and cuts out repeats of the word. However, it is case sensitive.
...
A word is a token in linguistics. Basically the unnest_tokens function pulls apart the unique words in the text. It is 647,091 of rows of unique words.
```{r}
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It is not so interesting because the article "The" pops up often in speeches or pieces of writing. The graphic also has connector and article words which are often used in speeches and writing pieces.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes, the list of words seem to be more interesting and unique.
Just a note on anti-join, ff the words match, then it does not join. If anything in the unique words don't match the stop words, then it is removed. So now we have a new dataframe of unique words with words we want to see. 
...
**Task** Run the following code and describe the results. **Answer** In 2021 the words "covid" and "health" is in the top ten list. Health popped up as the second word in 2022 but "covid" did not pop up in this list. 2022 also doesn't have the word "legislation" in the top ten words for the year whereas it popped up in the 2021 and 2023 list. The words "women" and "fire" made an appearance in the top ten for 2023 and did not appear in the top ten list for 2021 and 2022. 
...
**Task** Run the following code and describe the results. **Answer** The ngram function looks for words that are combined in the text. My understanding through the code is that it separates the bigrams first to have unique words and filter out the stopwords and then put them together again to make sure we have a more narrow down list of bigrams.

The bigrams like "covid 19" and "american rescue" makes sense. The bigrams gives more context of the text. Such as "american" and "rescue" separately mean something completely different from "american rescue". 
```{r}
releases |>
  filter(date < '2022-01-01') |>
...
**Task** Run the following code and describe the results. **Answer** Among the top single words in 2022 is "project" and "health" which also shows up as a bigram in the 2022 top two-word phrases such as "project location" and "health care".
...
**Task** Run the following code and describe the results. **Answer** An interesting two-word phrase that popped up in 2023 is "oral health" and although "health" is one of the top single word in 2023, "oral" was not in there. Oral health is included as one of the top two-word phrases because
it is a big policy issue for Cardin.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** I think sentiment analysis is quite tough to apply because you can't really attach sentiment to a single word without context. In my previous job in Malaysia, we used sentiment analysis on certain words to figure out if it is attached to hate speech in social media posts. We fed in a certain group of words and labelled them as positive and negative and the program we worked on would automatically pinpoint the sentiment for those words and it would in a way identify the post as hateful or not based on those sentiments. This may have been how the program was automated in identifying hate speech, but it needed people to come in and change the sentiment manually because the sentiment of the words may have changed based on real life context at the time too. This just reminds me of that situation.

Back to this dataframe, I think some of the words like 'urgent' listed as negative that I think could be neutral. I don't know if it is incorrect but how do we define and set in stone that certain words have negative or positive sentiments in general without any context? 
