...
**Answer** The term column describes the beginning of of each term chronologically in months from 2018. The count_classes column gives the number of classes in the terms. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** It organises the terms with the highest number of class listings in descending order. For example, the highest number of classes listed was in Fall 2022.
...
**Answer** The classes are grouped in departments. The code shows group by (term, department).
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The codeblock has added the column to the number of seats available for each class in each term. It does organise the term with the highest number of classes and, in a way, the highest number of seats in the classes in descending order.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** The data added two other columns that indicate the mean and median number of seats in the classes. Just to note, the data is arranged on terms with the highest number of classes and seats in descending order. The top terms are terms beginning in August, which are fall terms.
...
**Answer**
Two other columns have been added that organises the maximum and minimum number of seats in each department. The departments are arranged with the highest number of seats at the top going down in descending order.
### Task 12: Questions
...
1.  What are the names of the top 6 classes with the highest number of seats (in any department) and how many numbers of seats are there available each term for those classes?

2.  How many seats on the classes for each department for each term are filled? 

3.  What is the name of the professors from which department and term have teaches or has taught the classes with the highest number of seats filled arranged in descending order?
