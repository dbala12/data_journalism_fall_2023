...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer** The first line of the red warning message when the data is loaded says: One or more parsing issues, call `problems()` on your data frame for details, eg.: dat <- vroom (...). It essentially means that the data frame has encountered problems. A table of parsing issues or problems is created. My understanding is that tidyverse tries to guess the data type and the problems would pop up because tidyverse would think a piece of data that's supposed to be a character is recognised as a number like a zip code. 
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No. The warning does not pop up. I would guess that it is because the data in the vendor zip column is recognised as a character rather than a number.
...
**Answer** 549 zip codes. Some zip codes would have 4 digits because the first digit would be a 0 and excel or the records would not recognise a full 5 digit zip code with 0 in front.
...
**Answer**Each row explains the number of grants and loans (payments) by year. The group by choses the columns of Fiscal Year and category and the summarize counts the number of grants or loans issued in the fiscal year. The arrange function arranges the data according to Fiscal Year in ascending order. A couple of lines are problematic as the category may show NA (2010, 2011) and no loans for the fiscal year 2009 and category called L in 2013 and category called contract in 2017.
```{r}
md_grants_loans |> 
...
**Answer** The smallest amount of grants are $60 and $95.98 which is strange that there are grants lower than $100.
...
**Answer** It added the _ between the words in the column names. 
...
**Answer** The clean names function would standardize the column names and the rename function renames the column name grantor into source. The mutate function changes the characters/words in the source, grantee and description columns into upper case by using the str_to_upper() function. 
...
**Answer** I would think 50 duplicates possibly.
...
**Answer** The change is that the zip5 column has 5 digits in the zip codes and is standardized instead of the possible 9 digits or - in the zip codes in the zip_code column.
...
**Answer** The zip code that got the most amount of money from Commerce/Maryland Tourism Board is 21202. The zip code is in Baltimore and it makes sense to be the 
top recipient as it is the busiest (?) city with a high population. The total amount of money given by the Maryland Tourism Board to Baltimore is $648,069, which is the highest amount of money gotten by a zip code in Maryland.
```{r} 
cleaned_md_grants_loans |>
