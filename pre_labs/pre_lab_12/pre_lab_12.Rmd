...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** Firstly, it is different from Task 4 where there were columns with results, the lists and the list lengths whereas the Task 5 code makes sure the first dataframe is kept as the results and then shows that first dataframe as the results. So now, instead of showing that there are two types of results scraped in Task 4, one result with the press releases and the other the contact list, we have the results of the first dataframe of press releases. So the columns here on this dataframe are dates and press releases. The date column is a character column.
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** It's not so much information I want to add but it's to clean up the data and make sure there are no spaces in the rows and columns. Probably the next step would be to hyperlink the press release titles. 
...
tags there are in the HTML source? **Answer** There are multiple unordered lists according to the results below. 
...
tags. Describe the output: **Answer** html_text() gives the text inside all elements meaning the li. So this shows the actual text in the li elements. Not the li code and the text.
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** We have an actual dataframe due to the tibble function.
...
**Task** Run the following code in the gray-colored codeblock below to split the `value` column into two columns based on the ":" character and name them: *Note* Separate function takes the date column and the title values into separate columns. The output shows spaces after the date and it needs to be cleaned. The date needs to be changed too.
...
*Note* Dataframe that is clean with no space in both columns.
