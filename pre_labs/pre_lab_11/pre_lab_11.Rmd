...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It is case sensitive.
...
A word is a token in linguistics. Basically it is to pull apart the words. It is 647,091 of rows of unique words.
```{r}
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It is not so interesting because the article "The" would pop up often in speeches or pieces of writing. The graphic also has connectors and articles which are often used in speeches and writing pieces.
...
If the words match, then it does not join. If anything in the unique words don't match the stop words, then it is removed. So now we have a new dataframe of unique words with words we want to see. 
...
Some of the bigrams like covid19 and american rescue makes sense. The bigrams gives more context of the text. Such as american and rescue separately mean something completely different from american rescue. 
```{r}
releases |>
  filter(date < '2022-01-01') |>
...
**Task** Run the following code and describe the results. **Answer** oral health. A big policy issue for Cardin is adding dental into medical insurance coverage.
