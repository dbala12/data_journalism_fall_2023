...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** R has assumed that the top row with the Maryland data is the header, when it is not. There a lines of periods after the name of jurisdictions in the first column. Basically the data needs to be cleaned.
...
The headers are now x1, x2, x3, x4 and so on. But the good thing is the Maryland data is not the header anymore.
...
**Task** Run the codeblock below. What else do we need to do? **Answer** The lines of periods in the jurisdiction column needs to be removed.
...
```{r}
library(readr)
car_make <- read_csv("vin-raw-copy.csv") |>  clean_names()
View(car_make)
```
```{r}
library(readr)
car_make <- read_csv("vin-raw-copy.csv")
View(car_make)
```