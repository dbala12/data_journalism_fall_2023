...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are multiple different variations of names starting with the letter "w" and "W", in which some of the names are similar and can be categorized as the same but instead appear separately from each other. For example, "WASHINGTON CO COMMUNITY ACTION COUNCIL" and "WASHINGTON CO COMM. ACTION COUNCIL INC", where both could be the same grantee.
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** It changed Melwood Horticultural Training Center, Inc. in the Grantee column to Melwood Horticultural Training Center in the grantee_clean column. And it also changed The Tree House Child Advocacy Center of Montgomery County MD Inc.and Charles County Arts Alliance Inc in the Grantee column to THE TREE HOUSE CHILD ADVOCACY CENTER OF MONTGOMERY COUNTY MD INC.and 	CHARLES COUNTY ARTS ALLIANCE.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** WASHINGTON ADVENTIST UNIVERSITY in the Grantee column changed to 	Washington Adventist University in the grantee_clean column. WASHINGTON COLLEGE to 	Washington College. WASHINGTON COUNTY COMMISSION ON AGING to Washington County Commission on Aging. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** It really didn't make much changes. It looked similar to the results from key_collision_merge.
