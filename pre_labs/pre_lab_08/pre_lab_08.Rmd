...
There are 3822 rows and 4 columns. 
```{r}
...
Basically we are creating a new data frame using the Baltimore 911 overdose calls. In this new data frame, we are creating and adding a new month column using lubridate, where the "label=TRUE" gives us names or abbreviations of the months, instead of the months in numbers. For example, Jan or January instead of 01. We then calculate the total number of OD calls for each of these months. So the new data frame has 2 columns, that being months and the total number of OD calls for each month, and then the arrange function arranges the data based on the months in descending order with the highest total number of calls to the lowest total number of calls. 
```{r}
...
It is an empty gray box. I am guessing the ggplot() function has given us our blank canvas where we will build our charts on.
```{r}
baltco_911_calls_by_month |>
  ggplot()
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There is a bar chart on our blank canvas! So the geom_bar function has created a bar chart and the (aes(x=)) function, which is the aesthetics function has named the x-axis as the month that we called from the baltco_911_calls_by_month dataframe created. the bar charts are sized by the total calls per month. The total call per months is the y-axis, titled 'count'. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The bars, which are the total calls per month, are arranged in ascending order, from the months with the lowest total number of calls to the months with the highest total number of calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The coord_flip() function flipped the x and y axis with each other. 
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
The theme_minimal() feature has taken away the gray background of the blank canvas and given the bar chart a minimal "lool".
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** There is a blue background that is similar to the theme of bar charts in the Economist. The lines in the background is white and the bars are gray-ish. It looks a little nicer to look at, which would be able to attract the reader.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
A title has been given to our bar chart. The names/titles for the x and y axis have been changed to simpler names/titles too.
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A dataframe with the total calls by the date in a month. It is for everyday of the year.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There is a line graph based on the data frame created above. The line graph has an x-axis indicating the dates and the y-axis indicating the total calls for each date. Each point of the line chart is marked based on the number of total calls for the date. Honestly, this line graph shows that the calls made per date fluctuates quite a lot, but if we look at the background of the calls, it seems like the average number of calls made everyday is between 5 to 15 calls. It also looks like the number of calls are somehow decreasing towards the end of the year.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The change I see is that the x-axis. It seems like the scale_x_date function breaks the dates on a one week basis. To my understanding the date_labels = "%b %d" codes the date to the month abbreviation and then the date. But this has made the x-axis look very crowded. Also, I'm not sure if it is related but it seems like there are more background boxes. Is this because the x-axis has been scaled?
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** So the theme(axis.text.x = element_text (angle = 45, hjust=1)) has just been able to tilt the text elements in the x axis by 45 degrees and this makes it somewhat readable.
