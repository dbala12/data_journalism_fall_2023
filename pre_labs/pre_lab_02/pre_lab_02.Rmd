...
**Answer** The new variable is created to hold a specific and filtered data set we want to focus on for our analysis. This allows us to not need to write and repeat the code for the filtered data set (the new variable) for us to work on again.
...
**Answer:** The change made in Task 8 shows the expenditure_date column as a '<date>' instead of a character column. The mutate function has changed the expenditure_date column from a character to a date column by using the lubridate function mdy so that it recognises the date as month, day, year and turn it into a date that R would recognise as a date. The expenditure_date column in Task 8 would then show the date as recognised by R after the lubridate mutate function has been used.
...
**Answer** The mutate function has grouped by month the total amount of expenses for the month. The month column has a floor_date that attaches the total amount of expenses and grouped it to the 1st day of every month. The new 'month' column is arranged as such that the month with the highest total amount of expenses is at the top and going down in descending order.
...
**Answer** The code is creating two new collumns to the general_22 dataset. The mutate function is to change the dataset and add on two new columns with new data built on or based on data from an existing column. The first column creates the total votes of all candidates and this would be needed to create the next column which is the percentage of votes for Moore which is moore/total_votes calculated. Just to note, I think the decimal is in the pct_moore column because it has not been multiplied by 100 percent. Based on math it is the value over the total value multiplied by 100 percent that gives the percentage.
...
**Answer** If the change is from the previous task is Task 11, the data in the pct_moore column is arranged in descending order in counties with highest percentage of Moore votes going down to the lowest and this us due to the arrange function. However, if the change is based off from Task 10, then the change would be that the pct_moore column data in Task 12 is multiplied by 100 percent whereas pct_moore column data in Task 10 is not multiplied by 100 percent.Based on math it is the value over the total value multiplied by 100 percent that gives the percentage.
...
**Answer** It could be useful or interesting if what we are looking for are counties with the lowest percentage of Moore votes and then arranged in ascending order. 
...
**Answer** str_to_lower, this converts a character column to all lowercase. str_to_title, where this function would convert a string of characters in the column to make it look like a title, meaning the first letter of each word would be in uppercase and the other letters in the word in lowercase.
...
**Answer** The mutate statement is creating new columns in the data set where each column is assigned with a state and the addresses in the dataset are then categorised in the columns based on the state it is in. Say if there is an address in Maryland, it would be categorised under the new column as MD. It basically says that when a case of where an adress with Maryland in it is shown, then it would be recognised as MD in the new column.
...
**Answer** Yes it does. It shows that $77,723,145.82 was used in total expenses in Maryland. It would make sense that MD would have the highest total expenses as this is data is the Maryland Expenses with State.
...
1. In the Maryland expenditure data, what is the percentage of total amount expenses in states in descending order?

2. In the Maryland expenditure data, what is the percentage of total amount of expenses in states per month in descending order for payee type candidate committee?

3. In the Maryland expenditure data, which month had the highest total expenses by payee type self (candidate)?
